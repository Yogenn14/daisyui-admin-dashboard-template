...
const InventoryModal = ({
  ...
  const initializeFormData = () => {
    return poData.items.map((item) => ({
      ...
      conditions: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.condition)
        : [],
      statuses: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.status)
        : [],
      manufactureroems: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.manufactureroem)
        : [],
      inDates: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.inDate)
        : [],
      warrantyEndDates: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.warrantyEndDate)
        : [],
      suppliers: item.serialNumbers
        ? Array(item.serialNumbers.length).fill(item.supplier)
        : [],
    }));
  };
  ...
  const handleSubmit = async () => {
      ...
      items: formData.flatMap((item) => {
        ...
        if (item.type === "serialized") {
          return item.serialNumbers.map((serialNumber, index) => ({
            ...
            unitPrice:
              moc === "MYR" ? item.unitPrice / conversionRate : item.unitPrice,
            currency: moc,
          ...
          return {
            ...
            totalPrice:
              moc === "MYR"
                ? (item.quantity * item.unitPrice) / conversionRate
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_NODE_API_SERVER}inventory/validate-items`,
        {
          method: "POST",
          headers: {
          ...
          <div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-full md:w-3/4">
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            ...
                            {item.type === "serialized" ? (
                              Array.from({ length: item.quantity }).map(
                                (_, serialIndex) => (
                                  <div
                                    key={serialIndex}
                                    className="mb-4 p-4 border rounded-lg mt-4"
                                  >
                                    ...
                                    <input
                                      type="text"
                                      value={
                                        item.serialNumbers[serialIndex] || ""
                                      }
                                      onChange={(e) =>
                                        handleChange(
                                          index,
                                    ...
                                    <input
                                      type="text"
                                      value={
                                        item.manufactureroems[serialIndex] || ""
                                      }
                                      onChange={(e) =>
                                        handleChange(
                                          index,
                                    ...
                                    <input
                                      type="date"
                                      value={
                                        item.warrantyEndDates[serialIndex] || ""
                                      }
                                      onChange={(e) =>
                                        handleChange(
                                          index,
              ...
              {errors.length > 0 && (
                <>
                  <div
                    class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 mt-4"
                    role="alert"
                  >
                    <svg
                      ...
                      <div>
                        ...
                        {errors.map((error, idx) => (
                          <li key={idx} className="text-red-600">
                            Item {error.index + 1} ({error.field}):{" "}
                            {error.message}
                          </li>
                        ))}
                      </div>
                ...
                >
                  ...
                  <div>
                    <span class="font-medium">
                      Validation Successful! Please click next button to
                      continue
                    </span>
                  </div>
                </div>
